...
- **UI:** Phase 0 ✅ · **Phase 1 (map) ✅** · **Phase 2 (HUD & core screens) 🔨** — shipped: map
  HUD (menu bar + info panel), colony screen, Europe screen, Colony Advisor report, order buttons,
...
- **Phase 2 so far** ✅ — map HUD (reused `InGameMenuBar` + `ClassicInfoPanel`); **colony screen**
  ...
  (`ClassicEuropePanel`, `EUROPE.PIK`, recruit/purchase/train via the real controllers); **Colony
  Advisor report** (`ClassicReportColonyPanel`, `REPORT6.PIK`, first of nine reports); **order
  buttons** (reuse `FreeColAction` `BUTTON_IMAGE` art), **in-panel minimap**, **`WOODPANL.PIK` wood
  chrome**, and caption localization. (README "Phase 2 HUD" / "Colony screen" / "Europe screen" /
  "Report screens".)
...
1. **Other reports + page-through.** Eight `showReport*Panel` seams still no-op; add them over their
   `REPORTn.PIK` backdrops (naval/trade/religion/… — REPORT1–9 are extracted), and the original's
   page-through between column sets on the Colony Advisor report. Reuse the `ClassicReportColonyPanel`
   framing.
